...
public class SSAPBodyBulkItem {
	...
	public JsonNode getBodyAsJsonObject() {
		try {
			return JSON.deserializeToJson(this.body, true);
		} catch (IOException e) {
	...
	public String getBodyAsJsonString() {
		return JSON.jsonizeString(body);
    }
	...
	public static SSAPBodyBulkItem fromJsonToSSAPBodyBulkItem(String json) {
		try {
			return JSON.deserialize(json, SSAPBodyBulkItem.class);
		} catch (IOException e) {
			throw new SSAPMessageDeserializationError(e);
		}
	}
	...
	public static Collection<SSAPBodyBulkItem> fromJsonArrayToSSAPBodyBulkItems(String json) {
		try {
			return JSON.deserializeCollection(json, SSAPBodyBulkItem.class);
		} catch (IOException e) {
			throw new SSAPMessageDeserializationError(e);
		}
	}
